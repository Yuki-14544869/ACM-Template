...
Line2D line[4];
map <Point2D , int> mymap;
Polygon2D poly , covex;
int main () {
#ifndef ONLINE_JUDGE
    ...
    while (t --) {
        ...
        poly.n = 0;
        for (map <Point2D , int> :: iterator it = mymap.begin () ; it != mymap.end () ; it ++) {
            if (it -> second != 2) 
                ok = 0;
            poly.add (it -> first);
        }
        poly.getconvex (covex);
        if (!ok) {
            puts ("NO");
            continue;
        }
        if (covex.getarea () < 0.5) ok = 0;
        if (fabs ((covex.p[2] - covex.p[1]) * (covex.p[0] - covex.p[1])) > eps) ok = 0;
        if (fabs ((covex.p[1] - covex.p[0]) * (covex.p[3] - covex.p[0])) > eps) ok = 0;
        if (fabs ((covex.p[1] - covex.p[2]) * (covex.p[3] - covex.p[2])) > eps) ok = 0;
        
        int one = 0 , two = 0 , three = 0 , four = 0;
        for (int i = 0 ; i < 4 ; i ++) {
            int u = 0 , v = 0;
            for (int j = 0 ; j < 4 ; j ++) {
                if (line[i].a == covex.p[j]) {
                    u = j;
                }
            }
            for (int j = 0 ; j < 4 ; j ++) {
                if (line[i].b == covex.p[j]) {
                    v = j;
                }
            }
            if (u > v) swap (u , v);
